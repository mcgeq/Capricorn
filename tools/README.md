...
- Preferred template-init entrypoints live at the repository root:
  `run.bat` for Windows and `run.sh` for Unix-like systems.
- `rename_template.py`: Renames the sample project name, include directory,
  example file stem, sample namespace, and package name in a coordinated way.
  It can also report any remaining tracked template identifiers with `--check`,
  and supports an interactive `--init` mode for the common clone-then-enter-dir
  workflow. In `--init` mode it removes cloned `.git` metadata, runs `git init`,
  and removes template-maintainer files from the new project. The root wrapper
  scripts forward to this helper.
- `doctor.py`: Reports whether the local machine is ready for the template's
...
- `fix.py`: Applies the template's checked-in `clang-format` and `codespell`
...
- `test_template_init.py`: Creates a temporary copy of the repository, runs the
  initializer through the platform wrapper, and verifies that key files,
  identifiers, and paths were renamed coherently.
...
- `benchmark/` and `fuzz/`: Sample entry points showing how to wire Google
  Benchmark and libFuzzer into a project cloned from this template.
