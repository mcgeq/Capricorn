...
This template separates dependency management into three lanes:
...
The checked-in template follows that policy today:
...
If you want to apply the template's default local cleanups before committing,
...
- tests: `<binary-dir>/<ProjectName>_test`
- benchmarks: `<binary-dir>/<ProjectName>_benchmark`
- fuzzing: `<binary-dir>/<ProjectName>_fuzz`
- modules CLI: `<binary-dir>/<ProjectName>-modules`
...
need in order to resolve `import capricorn;` correctly.
...
If you prefer not to run binaries by path, the template provides helper targets:
...
The installed package exports `Capricorn::Capricorn` and can be consumed with
...
This template separates the sample code into a few lanes so it can grow into a
real library instead of getting stuck as a single-header demo:

- `include/Capricorn/capricorn.hpp` is the compatibility umbrella header.
- `include/Capricorn/core/` is the preferred home for focused public APIs.
...
3. The installed package exports a separate `Capricorn::modules` target when
   modules are enabled.
...
The template ships two `clang-tidy` profiles:
...
cmake -S . -B build/tidy `
  -D CMAKE_BUILD_TYPE=Debug `
  -D Capricorn_DEVELOPER_MODE=ON `
  -D Capricorn_ENABLE_CLANG_TIDY=ON `
  -D Capricorn_CLANG_TIDY_PROFILE=strict `
  -D Capricorn_CLANG_TIDY_WARNINGS_AS_ERRORS=ON `
  -D VCPKG_MANIFEST_FEATURES=test `
...
`benchmark` vcpkg feature. The template wires a small Google Benchmark sample
...
`fuzz` feature. The template assumes a Clang toolchain and links
...
When enabled, this template builds an additional `Capricorn::modules` target
from `source/modules/`.
